---
title: "04-统一接口与多厂商适配"
date: "2026-04-26T05:29:08.634Z"
lastUpdated: "2026-04-26T05:29:08.634Z"
...
model_precise = init_chat_model("gpt-4o-mini", model_provider="openai", temperature=0)

# temperature=0.3：轻微随机，偏向确定性但允许少量变化
# 适合文档总结、翻译、知识问答等
model_balanced = init_chat_model("gpt-4o-mini", model_provider="openai", temperature=0.3)

# temperature=0.7：标准随机，平衡确定性和多样性
# 适合日常对话、内容创作、头脑风暴
model_creative = init_chat_model("gpt-4o-mini", model_provider="openai", temperature=0.7)

# temperature=1.5+：高随机，输出非常多样化，可能出现不太连贯的内容
# 适合创意写作、诗歌生成等特殊场景，一般不推荐
model_wild = init_chat_model("gpt-4o-mini", model_provider="openai", temperature=1.5)
```
...
# 创意模式：适合头脑风暴、创意写作
model = init_chat_model("gpt-4o-mini", model_provider="openai", temperature=1.5)
```
...
for temp in [0, 0.7, 1.5]:
    ...
    results = [model.invoke(prompt).content for _ in range(3)]
...
```
temperature=0: ['生机勃勃', '生机勃勃', '生机勃勃']
temperature=0.7: ['温暖', '繁花', '希望']
temperature=1.5: ['绽放', '翠绿', '新生']
```

　　`top_p`（核采样）是另一种控制随机性的参数。它限制模型只从概率累计前 `top_p` 的 Token 中采样。`temperature` 和 `top_p` 通常只调一个，不建议同时修改。

### 4.2 最大 Token 数

　　`max_tokens` 限制单次生成的最大 Token 数量。一个 Token 大约相当于半个中文词或四分之三个英文单词。
...
response = model.invoke("详细介绍量子计算的原理")
print(response.content)
print(f"Token 使用量: {response.response_metadata['token_usage']}")
```

　　运行结果示例：

```
量子计算是一种利用量子力学原理进行信息处理的技术。与传统计算机使用二进制位（0或1）不同，量子计算使用量子位（qubit），它可以同时处于0和1的叠加态...
Token 使用量: {'prompt_tokens': 15, 'completion_tokens': 100, 'total_tokens': 115}
```

　　注意：设置过低的 `max_tokens` 会导致回答被截断。建议根据场景设置合理的上限：
...
|日常对话|500-1000|
|长文本生成|2000-4000|
|不限制（让模型自由发挥）|不设置（使用模型默认值）|

### 4.3 超时与重试

　　在生产环境中，网络波动和服务端过载是常态。合理的超时和重试配置是保障服务稳定性的关键：
...
def create_robust_model() -> BaseChatModel:
...
prompt = "为一家咖啡店写一条朋友圈推广文案"

configs = [
    {"temperature": 0, "max_tokens": 100, "label": "确定性短文案"},
    {"temperature": 0.7, "max_tokens": 200, "label": "平衡中文案"},
    {"temperature": 1.2, "max_tokens": 300, "label": "创意长文案"},
]

for cfg in configs:
    label = cfg.pop("label")
    model = init_chat_model(**base_config, **cfg)
    response = model.invoke(prompt)
    tokens = response.response_metadata.get("token_usage", {})
    print(f"\n{'='*50}")
    print(f"配置: {label} (temp={cfg['temperature']}, max={cfg['max_tokens']})")
    print(f"输出: {response.content}")
    print(f"Token: {tokens.get('completion_tokens', 'N/A')}")
```
...
```
==================================================
配置: 确定性短文案 (temp=0, max=100)
输出: 一杯好咖啡，开启美好一天。本店精选阿拉比卡咖啡豆，现磨现煮，欢迎品尝！
Token: 42

==================================================
配置: 平衡中文案 (temp=0.7, max=200)
输出: ☕ 生活需要仪式感，从一杯手冲咖啡开始。我们精选埃塞俄比亚耶加雪菲单品豆，花香果酸，层次分明。周末午后，来店里坐坐，让咖啡香气治愈你的疲惫。
Token: 87

==================================================
配置: 创意长文案 (temp=1.2, max=300)
输出: 在这个城市的某个角落，有一扇推开就会闻到咖啡香的门。我们相信，每一颗咖啡豆都藏着一个故事——从危地马拉的火山斜坡到你的杯中，这段旅程跨越了万公里。本周特调：桂花拿铁，秋天的味道被我们装进了一个杯子里。带上这本书，或者不带，来就好。
Token: 132
```

　　参数调优效果对比表：

|配置|temperature|max_tokens|风格|Token 消耗|适用场景|
| ------------| -----------| ----------| ------------| ----------| ------------------|
|确定性短文案|0|100|简洁直接|42|电商标题、短信推送|
|平衡中文案|0.7|200|生动有感染力|87|朋友圈、小红书|
|创意长文案|1.2|300|文艺有故事感|132|品牌文案、公众号|

## 五、常见陷阱与调试

### 5.1 API Key 未配置或配置错误
...
### 5.6 踩坑案例：不同平台的 max_tokens 上限不同
