---
title: "LangChain1.0生态全景与迁移指南"
date: "2026-04-26T01:28:16+08:00"
lastmod: "2026-04-29T00:05:07+08:00"
tags: ["人工智能", "Agent", "AI", "LangChain"]
lastUpdated: "2026-05-25T15:12:55.492Z"
description: "通过本章的学习，你将： 理解 LangChain 1.0 生态系统的全貌，包括七个核心组件和包结构设计 掌握 LangChain 从 0.x 到 1.0 的关键变化，建立版本迁移的清晰认知 了解 in..."
---
...
# 消息流模式：逐 Token 输出 AI 的文本回复（面向用户）
...
```
> **为什么消息流模式要加过滤？** `stream_mode="messages"` 会产出所有类型的消息块——包括 HumanMessage 块（用户输入）、ToolMessage 块（工具结果）、以及 AI 构建工具调用时的空内容块。只有 `AIMessageChunk` 且 `content` 非空时，才是真正面向用户的文本回复。不加过滤会导致输出混杂用户输入和垃圾空串。

两种模式的区别：**值流模式**（values）在每个执行步骤完成后输出中间数据，会显示完整的消息对象（含 tool_calls 等信息），适合调试和观察决策过程；**消息流模式**（messages）逐 Token 输出 AI 的纯文本回复，适合面向用户的聊天界面。在实际开发中，面向用户推荐消息流模式，调试阶段推荐值流模式。
#### 2.3.6 Agents：智能体

Agents 是前面六个组件的"集成器"。它以 LLM 为推理引擎，自主决策调用哪些工具、如何组织回答，形成 Think → Act → Observe 的闭环。Chain 中的执行步骤是预先定义的，但 Agent 中的执行步骤是模型自己决定的。
...
#### 2.3.7 Structured output：结构化输出

Structured output 解决的是 LLM 输出格式不可控的问题。LLM 默认返回自由格式文本，但实际应用中我们往往需要模型返回结构化的数据——比如情感分析的极性和置信度、用户信息提取的姓名和电话、代码审查的评分与建议。Structured output 让模型按照你定义的 Pydantic 模型或 JSON Schema 输出，保证每个字段都有、类型都对。

在 LangChain 1.0 中，结构化输出有两种使用方式：

**模型级别**：通过 `with_structured_output()` 方法，让任何 ChatModel 返回结构化数据。传入一个 Pydantic 模型类，模型会自动将其转换为 Provider 原生的结构化输出协议（如 OpenAI 的 JSON Schema 模式、Anthropic 的工具调用模式），返回值直接就是 Pydantic 对象：
...
class Sentiment(BaseModel):
    """文本情感分析结果。"""
    polarity: str = Field(description="情感极性：positive/negative/neutral")
...
result = structured_model.invoke("这款产品的用户体验非常棒，界面简洁！")
# result 是 Sentiment 对象：Sentiment(polarity='positive', confidence=0.92, reason='...')
```

**Agent 级别**：通过 `create_agent` 的 `response_format` 参数，让整个 Agent 的最终输出遵循指定结构。这在需要 Agent 执行多步推理后返回结构化结果的场景下特别有用：
...
agent = create_agent(
    model="gpt-4o",
    tools=[search_web],
    response_format=Sentiment,  # 传入 Pydantic 模型类
)

result = agent.invoke({
    "messages": [{"role": "user", "content": "分析一下这条评论的情感：太好用啦！"}]
})
# result["structured_response"] 是 Sentiment 对象
```

`with_structured_output()` 适合单轮调用的场景，`create_agent(response_format=...)` 适合需要工具调用或多步推理后输出结构化数据的场景。两者底层都依赖 Pydantic 模型定义，支持嵌套模型、可选字段、字段描述等完整特性。本系列第 07 篇将深入讲解 Pydantic 模型设计技巧、`method` 参数的三种模式（`json_schema`、`function_calling`、`json_mode`）以及 Agent 级结构化输出的三种策略。
