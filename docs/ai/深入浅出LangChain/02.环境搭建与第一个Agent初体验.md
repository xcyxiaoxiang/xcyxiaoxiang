---
title: "环境搭建与第一个Agent初体验"
date: "2026-04-23T17:33:57+08:00"
lastmod: "2026-04-29T00:04:40+08:00"
lastUpdated: "2026-05-24T15:49:36.933Z"
description: "欢迎来到 LangChain 实战的第一站。在上一篇中，我们理解了从 LLM 到 Agent 的演进逻辑——LLM 是\"大脑\"，工具是\"手脚\"，框架是\"神经系统\"。现在，我们要亲自动手搭建这条神经系统..."
---
...
![|700x350](/obsidian-assets/d9892bcff7f26672.webp)

![|700x350](/obsidian-assets/2a46a8fd47a46861.webp)
...
为了防止 API Key 泄露，务必在项目根目录创建 `.gitignore` 文件：
...
Model 是 Agent 的"大脑"，负责理解用户意图、决定是否调用工具、生成最终回答。LangChain 通过 `init_chat_model` 函数统一了不同模型提供商的接口。
...
这里的关键设计在于：`init_chat_model` 兼容所有 OpenAI API 格式的模型。通过 `base_url` 参数，我们将请求指向硅基流动，而不是 OpenAI 官方。这就是为什么前面说"切换提供商只需改 `.env`，代码不动"。

先用一个简单的脚本测试模型是否接入成功，创建 `02.demo_01_hello_agents.py`：

```python
# -*- encoding: utf-8 -*-
'''
@File : 02.demo_01_hello_agents.py
@Time : 2026/05/24 21:53:49
@Author : xcy.小相
@Version : 1.0
@Description : 02.环境搭建与第一个Agent初体验
'''
...
```bash
(agentdesign) ➜  agentDesign git:(master) ✗ uv run 02.demo_01_hello_agents.py 
类型: AIMessage
内容: 你好！我是Qwen，阿里巴巴集团推出的大规模语言模型，能够帮助你回答问题、创作文字、编程、分析数据等，支持多语言交流，并能根据需求提供实时信息和个性化服务。
Token 用量: {'input_tokens': 15, 'output_tokens': 172, 'total_tokens': 187, 'input_token_details': {'cache_read': 0}, 'output_token_details': {'reasoning': 127}}
```
...
基于文件 `02.demo_01_hello_agents.py`继续处理，这是一个完整的、可直接复制运行的 Agent 示例：
...
代码解析：
1. **导入部分**：`init_chat_model`（模型统一接入）、`@tool`（工具装饰器）、`create_agent`（Agent 构建 API）
...
在实际应用中，等待 Agent 完成全部思考再返回结果可能会让用户等很久。LangChain 提供了流式输出来解决这个问题，有两种常用模式：
...
```bash
uv pip show langchain langgraph langchain-core
uv pip install --upgrade langchain langgraph langchain-openai langchain-core
```

版本兼容性要求：`langchain >= 1.0.0`、`langgraph >= 0.4.0`、`langchain-openai >= 0.3.0`。如果你之前安装过 `langchain` 的 0.x 版本，建议先卸载再重装：

```bash
uv pip uninstall langchain langchain-core langgraph
...
1. **环境搭建**：使用 uv 或 pip 安装 `langchain`、`langgraph`，通过 `.env` 文件管理 API Key
