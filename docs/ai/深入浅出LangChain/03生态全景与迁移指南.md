---
title: "03-生态全景与迁移指南"
date: "2026-04-25T17:31:48.485Z"
lastUpdated: "2026-04-25T17:31:48.485Z"
description: "通过本章的学习，你将： 理解 LangChain 1.0 生态系统的全貌，包括六大核心组件和包结构设计 掌握 LangChain 从 0.x 到 1.0 的关键变化，建立版本迁移的清晰认知 了解 init_chat_model 统一接口的设计思想，为下一章实战打下基础 了解 LangChain 与 ..."
---

# 03-生态全景与迁移指南

# LangChain 1.0 生态全景与迁移指南
...
- 理解 LangChain 1.0 生态系统的全貌，包括六大核心组件和包结构设计
...
　　学完本章后，你将理解整个 LangChain 1.0 的包架构、六大核心组件的设计逻辑，以及从 0.x 迁移到 1.0 的关键要点，为后续深入学习每个组件打下基础。
...
　　LangChain 将 AI 应用开发拆解为六个核心模块。每个模块专注一项能力，可独立使用，也可自由组合。下图展示了六大组件之间的关系：

```mermaid
flowchart TD
    AGENT["🤖 Agents<br/>智能体<br/>自主决策与执行"]
    MODEL["🧠 Models<br/>模型连接器<br/>统一接口调用 LLM"]
    PROMPT["📝 Prompts<br/>提示词管理<br/>动态模板与变量"]
    TOOL["🔧 Tools<br/>工具集成<br/>执行外部动作"]
    MEM["💾 Memory<br/>记忆机制<br/>对话历史持久化"]
    RET["📚 Retrieval<br/>检索增强<br/>RAG 向量检索"]
...
　　上图描述了 LangChain 1.0 六大核心组件的关系：Agent 处于中心位置，其余五个组件（Models、Prompts、Tools、Memory、Retrieval）作为能力源向 Agent 提供支持。虚线表示运行时数据流，实线表示能力供给关系。
...
qwen_model = init_chat_model("qwen3-8b", model_provider="siliconflow")
...
　　Prompts 模块帮你动态生成和管理提示词。核心功能是"提示词模板"——定义一个带变量的模板，运行时填入具体值。它让你的提示词从硬编码变成可配置、可复用的资产。
...
chain = prompt | model
result = chain.invoke({"role": "翻译专家", "style": "专业", "question": "如何学习Python？"})
```

```bash
# 运行结果示例
作为翻译专家，我建议按照以下步骤学习 Python：1. 从基础语法开始...
```

　　上面代码中的 `prompt | model` 使用了 LCEL（LangChain Expression Language）语法，通过管道操作符 `|` 将提示词模板和模型串联成数据流管道。LCEL 是 LangChain 底层的运行机制，后续章节会专门讲解。
...
　　通过 `thread_id` 实现会话隔离，不同用户或不同对话线程互不干扰。

#### 2.3.5 Retrieval：检索增强

　　Retrieval 模块实现了 RAG（Retrieval-Augmented Generation，检索增强生成）技术——把私有文档切分成小块，转换成数学向量，存入向量数据库。当用户提问时，先检索最相关的文档段落，再把问题和文档一起交给模型，让模型基于私有数据来回答。

　　RAG 解决了 LLM 的两个核心痛点：**知识时效性**（LLM 的训练数据有截止日期）和**私有数据访问**（LLM 无法访问企业内部文档）。本系列将在第 21-25 篇深入讲解 RAG 的完整流程。
...
model = init_chat_model("qwen3-8b", model_provider="siliconflow")
```
...
1. **生态系统**：LangChain 1.0 包含六个核心组件——Models、Prompts、Tools、Memory、Retrieval、Agents，它们围绕 Agent 这个中心提供能力支持
